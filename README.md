...
Mini App разделён табами на три экрана: **Лидерборд** (`/miniapp/leaderboard`, открывается по умолчанию — список участников активного события, отправивших результат, с местами, полным и чистым временем; тап по строке открывает карточку со всеми метриками результата), **Призы** (`/miniapp/gifts`, каталог одобренных подарков) и **Мои призы** (`/miniapp/my-gifts`). В «Моих призах» даритель видит все добавленные им призы активного события, включая ожидающие проверки; для призов, которым администратор включил ручное распределение, он выбирает, меняет или очищает получателя. Ручное назначение и оба случайных действия доступны только для участника того же события с актуальным результатом или статусом DNF; DNS, участники без результата и дисквалифицированные исключаются.

Для одобренного ручного приза без получателя доступны два случайных назначения: «Отдать рандомному участнику без награды» выбирает только участника без автоматических и ручных призов; «Отдать рандомному участнику» допускает уже награждённого участника.
